...
调用；实际执行代码位于 `src/apimart_h3_pipeline` 包中。

包内只保留两级职责分层：`core/`、`providers/`、`bridge/`、`execution/`、
`media/` 和 `resources/`。不再为了单个 helper 继续增加目录，根包中的少量
兼容模块只负责转发旧导入。
...
- `execution/artifacts.py` 负责 attempt 归档、observer 记录和 manifest 更新。
...
`src/apimart_h3_pipeline/resources/prompts/`。它们不是 Python 字符串常量，而是随包发布
